...
protected:
	...
	void send_connacc() {
		...
		if (this->protocol == "tcp") {
			if (send(this->client_fd, &conacc_packet, sizeof(conacc_packet), 0) < 0) {
	...
	void receive_and_send_confirmations() {
		...
		CONN conn_packet;
		if (recv(this->client_fd, &conn_packet, sizeof(conn_packet), 0) < 0) {
	...
	void send_rjt_packet() {
		...
		if (send(this->client_fd, &rjt_packet, sizeof(rjt_packet), 0) < 0) {
			throw std::runtime_error("ERROR: send failed\n");
		}
	}
	...
	void get_and_validate_data_header(DATA_HEADER &data_header) {
		if (recv(this->client_fd, &data_header, sizeof(data_header), 0) < 0) {
			throw std::runtime_error("ERROR: recv failed\n");
		}
		if constexpr (DEBUG) {
	...
	void send_acc(DATA_HEADER &data_header) {
		if (this->protocol == "udpr"){
			...
			if (send(this->client_fd, &acc_packet, sizeof(acc_packet), 0) < 0) {
				throw std::runtime_error("ERROR: send failed\n");
			}
		}
	}
	...
	void receive_data() {
		while (payload_length > 0) {
			...
			while (current_data_length < data_header.data_length){
				...
				int64_t data_received = recv(this->client_fd, buff.get()+current_data_length, data_header.data_length-current_data_length, 0);
				if (data_received < 0) {
					throw std::runtime_error("ERROR: recv failed\n");
				}
				if (data_received == 0){
					throw std::runtime_error("ERROR: recv failed. No data received\n");
				}
				current_data_length += data_received;
			}
	...
	void confirm_data() {
		...
		if (send(this->client_fd, &rcvd_packet, sizeof(rcvd_packet), 0) < 0) {
			throw std::runtime_error("ERROR: send failed\n");
		}
		if constexpr (DEBUG) {
...
public:
	Server(std::string protocol, int port) : buff{new char[64000]} {
		...
		if (this->protocol != "tcp" && this->protocol != "udp" && this->protocol != "udpr") {
			throw std::runtime_error("ERROR: invalid protocol. Protocols supported: <tcp>, <udp>, <udpr>\n");
		}
		if (this->protocol == "tcp") {
